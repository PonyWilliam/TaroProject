...
import chouqianStyleSheet from "./chouqian_styles";
var _styleSheet = chouqianStyleSheet;
let Mine = class Mine extends Component {
  ...
  render() {
    ...
    if (this.state.showbutton) {
      ...
      button = <Button onClick={this.chou.bind(this)} style={_styleSheet["button"]}>抽签</Button>;
    } else {
      button = <View></View>;
    }
    if (this.state.isOpened) {
      res = <AtCurtain isOpened={true}>
        <Image style="width:100%;height:250px" src="https://qq-1257689370.cos.ap-guangzhou.myqcloud.com/qian/qian.jpg" onClick={this.chou2.bind(this)} />
    </AtCurtain>;
    }
    if (this.state.haveresult) {
      content = <ClAnimation type="scale-top" delay={0} duration={0.3}>
        <AtCard title={this.state.result.title} thumb="http://www.logoquan.com/upload/list/20180421/logoquan15259400209.PNG" style={_styleSheet["card"]}>
            {this.state.result.content}
        </AtCard>
        </ClAnimation>;
    }
    return <View style={_styleSheet["chouqian"]}>
            {button}
            {res}
            {content}
