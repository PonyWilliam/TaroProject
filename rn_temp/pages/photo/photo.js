...
import { AtImagePicker, AtNoticebar, AtButton } from 'taro-ui';
import { ClAnimation } from 'mp-colorui';
import photoStyleSheet from "./photo_styles";
var _styleSheet = photoStyleSheet;
...
let Mine = class Mine extends Component {
  ...
  render() {
    ...
    if (this.state.add == false) {
      button = <View style={_styleSheet["submit"]}><AtButton type="primary" size="normal" circle onClick={this.submit.bind(this)}>提交</AtButton></View>;
    } else {
      button = <View></View>;
    }
    if (this.state.download == true) {
      result = <View style={_styleSheet["result"]}>
          <View style={_styleSheet["download"]}>
            <View style={_styleSheet["button"]}>
          ...
          <Text style={_styleSheet["preview"]}>结果预览</Text>
    ...
    return <View style={_styleSheet["photo"]}>
          <ClAnimation type="slide-top" delay={0} duration={0.6}>
            <AtNoticebar icon="volume-plus" marquee>
                证件照上传后可以得到一个返回的图片存储到手机。存储到手机需要观看一定时长的广告。
            </AtNoticebar>
            <AtImagePicker length={1} showAddBtn={this.state.add} files={this.state.files} onChange={this.onChange.bind(this)} onFail={this.onFail.bind(this)} onImageClick={this.onImageClick.bind(this)} />
            {button}
            {result}
